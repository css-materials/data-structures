...
# everything typed after the # is a comment
...
x <- 5    # store value 5 into object x
X<-5      # spacing does not matter
x = 5     # can use = but better to use <-
...
# display the object and check its type using typeof() or class()
my_int
typeof(my_int)
class(my_int)

# here is an example of using objects
result <- my_int + 2
print(result)
result*2

# convert my_double to logical and save it as "a"
typeof(my_double)
b <- as.logical(my_double)
typeof(b)

# if you cannot convert among data type: error!
# for example you cannot convert a character to a numeric or double 
my_char
c <- as.numeric(my_char)

# but you can do the other way around (numeric or double to character)
my_double
...
# remove the numeric object "my_double" from memory
rm(my_double)

# remove selected objects from memory
rm(my_int, my_char)

# remove all objects beside one
...
# To remove all objects from your environment and start fresh I suggest the following
# 1. Session > Restart R
# 2. do not preserve your workspace
...
named_list <- list(a = (c(1,2,3)), 
                   b = c("ciao", "bye"), 
                   c = 3.3)
named_list

# use [] to access elements
# important; results will remain a list regardless on the object
named_list[1]
named_list[2]
named_list[3]
named_list[4]
...
# important: results wont be a list, and will vary according to the object
named_list[[1]]   # 1st element by index
named_list[["a"]] # 1st element by name
named_list[[2]]   # 2nd element by index
named_list[["b"]] # 2nd element by name
